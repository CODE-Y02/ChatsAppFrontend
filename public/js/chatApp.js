...
async function sendMsgToServer(message, token) {
  try {
    ...
    let res = await axios.post(
      "http://43.205.208.218:3000/message/send",
      {
        message: message,
        groupId,
      },
...
async function fetchAllOrLatestMsg(token) {
  try {
    ...
    let response = await axios.get(
      `http://43.205.208.218:3000/message/getall?lastmessageId=${lastMsgId}`,
      {
        headers: {
          authorization: token,
        },
...
async function fetchGroups(token) {
  try {
    let response = await axios.get(
      `http://43.205.208.218:3000/group/all`,
...
async function fetchGroupMsg(id) {
  try {
    ...
    let response = await axios.get(
      `http://43.205.208.218:3000/message/group/${id}/?lastmessageId=${lastMsgId}`,
      {
        headers: {
          authorization: token,
        },
